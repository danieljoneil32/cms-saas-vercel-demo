import 'server-only'
import { type HeroElementDataFragment } from "@/gql/graphql"
...
import { DefaultHeroElementComponent } from './displayTemplates'
...
export const HeroElement : DefaultHeroElementComponent<HeroElementDataFragment> = ({ data: { altText, HeroLink }, layoutProps, children, ...props }) => {
    const { 
        roundedCorners="none", 
        appear="none", 
        delay="none", 
    ...
    if (useFadeIn) {
        ...
        >
            <Image alt={altText ?? ""} src={ HeroLink } fill className="object-cover" />
        </Animation>
    }
